...
export default function TodayView({ session }: TodayViewProps) {
  ...
  // Net spending = expenses minus any income received today (income offsets the daily spend)
  const todayNetSpending = Math.max(0, todaySpending - todayIncome)
  const status = getSpendingStatus(todayNetSpending, dailyBudget)
  const greeting = getGreeting()
  ...
  async function handleDelete(id: string) {
    try {
      ...
      // Refresh data and allow ML to re-run with updated transactions
      mlFetchedRef.current = false
      setMlStatus(null)
      setMlForecast(null)
      setMlInsight(null)
        ...
        <div className="card-glass p-5 space-y-4">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="font-mono text-2xl font-bold text-accent">{formatCurrency(todayNetSpending)}</p>
              {todayIncome > 0 && (
                <p className="text-xs text-success mt-1">
                  +{formatCurrency(todayIncome)} pendapatan hari ini
                </p>
              )}
            </div>
            <StatusBadge status={status} />
          </div>
          <ProgressBar spent={todayNetSpending} limit={dailyBudget} />
          <div className="flex justify-between text-xs text-text-secondary pt-2 border-t border-accent/5">
            <span>Batas: {formatCurrency(dailyBudget)}</span>
            <span>Sisa: {formatCurrency(Math.max(0, dailyBudget - todayNetSpending))}</span>
          </div>
        </div>
